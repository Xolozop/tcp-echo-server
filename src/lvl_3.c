...
static void log_error(const char *fmt, int err, ...) {
    va_list ap;
    va_start(ap, err);
...
static void handle_signal(int sig) {
    log_msg("Received signal %d (%s), shutting down...", sig, strsignal(sig));
    server_running = 0;
}
...
/* эхо zero-copy через splice */
static ssize_t zero_copy_echo(int client_fd) {
    int pipe_fd[2];
    ssize_t total_bytes = 0;
    
    /* pipe для передачи данных внутри ядра */
    if (pipe2(pipe_fd, O_NONBLOCK) < 0) {
        return -1;
    }
    
    /* Splice: данные из сокета в pipe */
    ssize_t n = splice(client_fd, NULL, pipe_fd[1], NULL, SPLICE_CHUNK_SIZE,
    ...
    /* Splice: данные из pipe обратно в сокет */
    ssize_t sent = splice(pipe_fd[0], NULL, client_fd, NULL, n,
                          SPLICE_F_MOVE | SPLICE_F_MORE | SPLICE_F_NONBLOCK);
    ...
    /* Возможно в буфере остались данные */
    if (n == SPLICE_CHUNK_SIZE) {
        return 0;
    }
...
/* Zero-Copy обработка клиента (эхо) - данные не попадают в user space */
static void handle_client_zero_copy(int client_fd) {
    ssize_t n;
    int total_bytes = 0;
    int zero_copy_rounds = 0;
    
    log_msg("Client connected (zero-copy mode): fd=%d", client_fd);
    
    while (1) {
        n = zero_copy_echo(client_fd);
        
        if (n > 0) {
            total_bytes += n;
            zero_copy_rounds++;
        } else if (n == 0) {
            /* Клиент закрыл соединение или данных больше нет */
            if (total_bytes > 0) {
                log_msg("Zero-copy stats: fd=%d, rounds=%d, total=%d bytes",
                        client_fd, zero_copy_rounds, total_bytes);
            }
            close(client_fd);
            return;
        } else {
            /* Ошибка или нет данных для чтения */
            if (errno == EAGAIN || errno == EWOULDBLOCK) {
                /* Временная задержка, сокет будет обработан снова через epoll */
                break;
            } else if (errno != EINTR) {
                log_error("zero_copy_echo() failed for fd=%d", client_fd, errno);
                close(client_fd);
                return;
            }
        }
    ...
    log_msg("Connection processed (zero-copy): fd=%d (total echoed: %d bytes, rounds: %d)",
            client_fd, total_bytes, zero_copy_rounds);
    
    /* Возвращаем сокет в epoll для следующих событий */
    struct epoll_event ev;
    ev.events = EPOLLIN | EPOLLET | EPOLLONESHOT;
    ev.data.fd = client_fd;
    if (epoll_ctl(epoll_fd, EPOLL_CTL_MOD, client_fd, &ev) < 0) {
        log_error("epoll_ctl(MOD) failed for fd=%d", client_fd, errno);
        close(client_fd);
    }
}
...
static void add_task(int client_fd) {
    ...
    if (!task) {
        log_error("malloc() failed for task", errno);
        close(client_fd);
        return;
    }
...
static void* worker_thread(void* arg) {
    ...
    while (server_running) {
        ...
        if (client_fd >= 0) {
            handle_client_zero_copy(client_fd);
        }
    }
    return NULL;
}
...
int main(int argc, char** argv) {
    ...
    if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, listen_fd, &ev) < 0) {
        log_error("epoll_ctl(ADD) failed for listen_fd", errno);
        close(listen_fd);
        close(epoll_fd);
        close_log_file();
        return EXIT_FAILURE;
    ...
    for (int i = 0; i < THREAD_POOL_SIZE; i++) {
        if (pthread_create(&worker_threads[i], NULL, worker_thread, NULL) != 0) {
            log_error("pthread_create() failed for worker %d", i, errno);
            server_running = 0;
            break;
        }
    }
    ...
    log_msg("Workers: %d", THREAD_POOL_SIZE);
    log_msg("Zero-copy: splice() (kernel → kernel)");
    log_msg("Chunk size: %d bytes", SPLICE_CHUNK_SIZE);
            ...
            if (fd == listen_fd) {
                ...
                while (1) {
                    ...
                    if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, client_fd, &ev) < 0) {
                        log_error("epoll_ctl(ADD) failed for client_fd=%d", client_fd, errno);
                        close(client_fd);
                        continue;
                    }
